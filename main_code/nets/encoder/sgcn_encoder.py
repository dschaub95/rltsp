...
# taken from https://github.com/liangxinedu/NeuroLKH/blob/main/net/sgcn_layers.py
...
class NodeFeatures(nn.Module):
    ...
    def forward(self, x, e, edge_index, n_edges):
        ...
        Vx = Vx[
            torch.arange(batch_size).view(-1, 1), edge_index
        ]  # batch_size x n_node * n_edge x hidden_dimension
        # Vx = torch.matmul(Vx.permute(0, 2, 1), adj_to).permute(0, 2, 1)
        to = Ve * Vx
        to = to.view(batch_size, num_nodes, n_edges, hidden_dim).sum(
            2
        )  # batch_size x n_node x hidden_dimension
        x_new = Ux + to
        if self.is_pdp:
...
class EdgeFeatures(nn.Module):
    ...
    def forward(self, x, e, edge_index, inverse_edge_index, n_edges):
        ...
        inverse_Ue = self.inverse_U(
            e
        )  # batch_size x n_node * n_edge x hidden_dimension
        inverse_Ue = torch.cat(
            (
                inverse_Ue,
                self.W_placeholder.view(1, 1, hidden_dim).repeat(batch_size, 1, 1),
            ),
            1,
        )  # batch_size x (n_node * n_edge + 1) x hidden_dimension
        ...
        Vx = Vx.view(batch_size, -1, n_edges, 128) + Vx_from.view(
            batch_size, -1, 1, 128
        )
...
class SparseGCNLayer(nn.Module):
    ...
    def forward(self, x, e, edge_index, inverse_edge_index, n_edges):
        ...
        e_tmp = self.edge_feat(
            x_new, e_in, edge_index.long(), inverse_edge_index.long(), n_edges
        )  # batch_size x n_node * n_edge x hidden_dimension
