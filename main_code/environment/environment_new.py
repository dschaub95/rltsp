...
class MultiGroupState(GroupState):
    def __init__(self, state_list):
        ...
        self.selected_node_list = torch.cat([self.pad3d_left(state.selected_node_list, max_count) for state in state_list], dim=0)
        self.ninf_mask = torch.cat([state.ninf_mask for state in state_list], dim=0)
        self.current_node = torch.cat([state.current_node for state in state_list], dim=0)
    ...
    def split(self):
        ...
        for k, selected_count in enumerate(self.selected_count):
            ...
            state.ninf_mask = self.ninf_mask[k*orig_batch_s:(k+1)*orig_batch_s,:,:]
            # recover the true selected node tensor
            raw_selected_node_list = self.selected_node_list[k*orig_batch_s:(k+1)*orig_batch_s,:,:]
            num_selected_nodes = raw_selected_node_list.size(-1)
            # recover single selected node list
            state.selected_node_list = torch.unique_consecutive(raw_selected_node_list, dim=-1)[:,:,0:self.tsp_size]
